...
async function ensureStorageDownloadUrl(objectPath: string): Promise<string> {
...
function parseGdriveRef(ref: string): { fileId: string; fileName?: string } | null {
  if (!ref.startsWith(GDRIVE_PREFIX)) return null;
...
async function assertCoachOwnsRef(uid: string, ref: string) {
  const snap = await getFirestore().collection('coach_registration_requests').doc(uid).get();
  const urls = (snap.data()?.certificateUrls as string[] | undefined) ?? [];
...
export const uploadCoachDocument = onCall(
  ...
  async (request) => {
    ...
    const fileName = request.data?.fileName as string | undefined;
    const contentType =
      (request.data?.contentType as string | undefined)?.trim() || 'application/octet-stream';
    const dataBase64 = request.data?.dataBase64 as string | undefined;
...
export const getCoachDocumentDownloadUrls = onCall(async (request) => {
  ...
  const uid = request.auth.uid;
  const refs = request.data?.refs as string[] | undefined;
  ...
  if (!coachDocumentsDriveFolderId.value().trim()) {
    throw new HttpsError(
      'failed-precondition',
      'Старые файлы в Google Drive недоступны. Загрузите документы заново.',
    );
  }
